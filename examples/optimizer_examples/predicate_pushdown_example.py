...
# Create a LazyFrame with semantic filter followed by pandas filter
...
lf_after = lf_before.optimize([PredicatePushdownOptimizer()])
...
print("-" * 80)
print("Before:")
for i, node in enumerate(lf_before._nodes):
    print(f"  {i}. {type(node).__name__}")

print()
print("After:")
for i, node in enumerate(lf_after._nodes):
    print(f"  {i}. {type(node).__name__}")

print()
print("=" * 80)
print("The filter node has been moved earlier in the LazyFrame!")
print("=" * 80)
