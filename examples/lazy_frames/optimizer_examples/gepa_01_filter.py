...
# ---------------------------------------------------------------------------
# Build the initial (unoptimized) LazyFrame
# ---------------------------------------------------------------------------
...
lf = LazyFrame().sem_filter("{review} is an extremely enthusiastic positive review with strong praise")
...
optimizer = GEPAOptimizer(
    ...
    valset=reviews_df.iloc[5:].reset_index(drop=True),
)

optimized_lf = lf.optimize([optimizer], train_data=reviews_df.iloc[:5].reset_index(drop=True))
...
print("\n" + "=" * 70)
print("Filtered reviews (optimized LazyFrame):")
print("=" * 70)
